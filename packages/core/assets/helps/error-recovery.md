...
It is not cosmetic. Every later reader runs the strict tier: `getItems` returns
the same finding as a `warning`, and publishing a shared app REFUSES exactly
these rows. A batch generated by a script is wrong in every row the same way, so
