<template>
  <div class="flex items-center gap-2 relative" :class="fluid ? 'min-w-0 flex-1' : 'w-56 shrink-0'">
    <button
      ...
      @click="open = !open"
    >
      <span class="material-icons text-base text-gray-500" :class="ROLE_ICON_CONTAINMENT">{{ roleIcon(roles, currentRoleId) }}</span>
    ...
    <div v-if="open" ref="dropdown" class="absolute left-0 right-0 top-full z-50 bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden">
      ...
      >
        <span class="material-icons text-base text-gray-400" :class="ROLE_ICON_CONTAINMENT">{{ roleIcon(roles, role.id) }}</span>
        {{ role.name }}
      </button>
    </div>
  </div>
...
import { ROLE_ICON_CONTAINMENT, roleIcon, roleName } from "../utils/role/icon";
