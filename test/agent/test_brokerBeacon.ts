...
describe("deliverBeacon", () => {
  ...
  // The caller is `initialize`'s fire-and-forget path: a rejection there would
  // become an unhandled rejection in the broker, not a logged beacon failure.
  it("never rejects, even when the reporter itself throws", async () => {
